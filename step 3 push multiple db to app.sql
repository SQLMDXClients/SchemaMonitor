...
BEGIN
    ...
    SQLSTMT := 'SELECT FAMILY_NAME, DATABASE_NAME, ENVIRONMENT FROM _METADATA.SNOWFLAKE_TARGET_DATABASES WHERE DEFAULT_PULL';

    RES := (EXECUTE IMMEDIATE :SQLSTMT);

    LET DATABASE_CURSOR CURSOR FOR RES;

    FOR RECORD IN DATABASE_CURSOR DO
    ...
    IF (:DATABASE_NAME IS NULL) THEN
        RETURN 'YOU NEED TO SNOWFLAKE_SET_DATABASE.  NONE ARE AVAILABLE';
    ELSE 
        RETURN 'STAGE DATABASE ' || :DATABASE_NAME || ' METADATA TO SCHEMAMONITOR';
    END IF;
