...
--   CALL _CODE.SNOWFLAKE_SET_TARGET_DATABASE(CURRENT_ORGANIZATION_NAME(), 
--        CURRENT_ACCOUNT_NAME(), 'SAMPLE_DATA', 'SAMPLE_DATA_DEV', 'DEV', TRUE);

CALL _CODE.SNOWFLAKE_SET_TARGET_DATABASE(CURRENT_ORGANIZATION_NAME(), 
     CURRENT_ACCOUNT_NAME(), '<<FAMILY_NAME>>', '<<DATABASE_NAME>>', 
     '<<ENVIRONMENT>>', TRUE);
